...
export default class TransferQRScreen extends React.Component {
  constructor(props) {
    super(props);
      ...
      if (String(user.id) !== id) {
        ...
        if (response.status === 200) {
          ...
          if (result.user) {
            ...
            Alert.alert('Transferencia rápida', 'Escanea un QR válido.');
          }
        } else {
          Alert.alert('Transferencia rápida', 'Escanea un QR válido.');
        }
  ...
  async onPressSubmit() {
    try {
      ...
      if (concept === '') {
        ...
        Alert.alert(
          'Transferencia rápida',
          'Escanea el código QR del usuario al que se realizará la transferencia.',
        );
      } else if (quantity === '') {
        Alert.alert(
          'Transferencia rápida',
          'Ingresa la cantidad de saldo a transferir.',
        );
      } else if (nip === '') {
        Alert.alert(
          'Transferencia rápida',
          'Ingresa el nip para validar la transacción.',
        );
      } else if (user === null) {
        Alert.alert(
          'Transferencia rápida',
          'Ocurrió un problema para acceder realizar la transacción.',
        );
      } else {
        ...
        if (response.status === 200) {
          ...
          if (result.message === 'Successfully transfered.') {
            ...
            Alert.alert('Transferencia rápida', result.message);
          }
        } else {
          Alert.alert(
            'Transferencia rápida',
            'Hubo un problema para realizar la transferencia.',
          );
        }
      }
    } catch (error) {
      Alert.alert(
        'Transferencia rápida',
        'Hubo un problema para realizar la transferencia. Intente más tarde.',
      );
    }
  }
  ...
  render() {
    ...
    return (
      <Container>
        {/* Screen header */}
        <AppHeader hasBack nested={true} title="Transferencia rápida" />
          ...
          {isLoading ? (
            ...
            <>
              ...
              <NeuButton
                ...
                style={styles.button}
                width={150}>
                <Text style={[styles.text, styles.submit]}>Transferir</Text>
              </NeuButton>
