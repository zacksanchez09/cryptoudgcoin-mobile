...
import {OXXO_RECHARGE_INFORMATION_URL} from '../utils/API';

class TransactionOxxoScreen extends Component {
  constructor(props) {
    super(props);
    ...
    this.getOxxoPay = this.getOxxoPay.bind(this);
  ...
  async getOxxoPay() {
    ...
    const response = await fetch(`${OXXO_RECHARGE_INFORMATION_URL}/${item.id}`);

    if (response.status === 200) {
      const result = await response.json();
      console.log(result);

      this.setState({oxxo_pay: result});
    } else {
  ...
  render() {
    ...
    return (
      <Container>
        {/* Screen header */}
        <AppHeader title="OXXO Pay" hasBack />
...
export default TransactionOxxoScreen;
